...
create policy if not exists "Users can insert their own referral record"
  on public.referrals
  for insert
  to authenticated
...
-- Note: Postgres doesn't support IF NOT EXISTS for ADD TABLE, so we wrap in DO blocks
DO $$ BEGIN
  BEGIN
