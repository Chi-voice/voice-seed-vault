...
SELECT 
  ...
  CASE sequence_order
    ...
    WHEN 11 THEN 'Asking about someone\'s wellbeing'
    WHEN 12 THEN 'Asking for someone\'s identity'
    ...
    WHEN 17 THEN 'Stating one\'s place of residence'
...
WHERE NOT EXISTS (
  ...
  WHERE t.language_id = l.id AND t.is_starter_task = true AND t.sequence_order = generate_series.generate_series
);
...
BEGIN
  ...
  -- Update user's total recordings and award points
